...
class Net(nn.Module):
    def __init__(self, init):
        ...
        self.gmm_layer = lcl_models.axis_aligned_gmm_layer(128, dim, init)
...
def main(init):
    ...
    model = Net(init=init).to(device)

    checkpoint = torch.load('/home/mmajurski/github/ssl-gmm/models-20230312/adam-softmax/model.pt')
    model.load_state_dict(checkpoint.state_dict(), strict=False)
    model = model.to(device)

    # set the entire model to frozen
    for param in model.parameters():
        param.requires_grad = False

    # unfreeze the gmm weights, so only those can learn
    for param in model.gmm_layer.parameters():
    ...
    output_folder = './models-20230312/adam-frozen-gmm-{}-128dim'.format(init)
