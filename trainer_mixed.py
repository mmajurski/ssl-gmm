...
class SupervisedTrainerMixed(trainer.SupervisedTrainer):
    ...
    def train_epoch(self, model, pytorch_dataset, optimizer, criterion, epoch, train_stats, nb_reps=1, unlabeled_dataset=None):
        ...
        for rep_count in range(nb_reps):
            for batch_idx, tensor_dict in enumerate(dataloader):
                ...
                with torch.cuda.amp.autocast(enabled=self.args.amp):
                    resp_gmm, resp_cmm = model(inputs)
                    batch_loss_gmm = criterion(resp_gmm, labels)
                    batch_loss_cmm = criterion(resp_cmm, labels)
                    # batch_loss = (batch_loss_gmm + batch_loss_cmm) / 2.0
                    batch_loss = batch_loss_cmm
                    outputs = (resp_gmm + resp_cmm) / 2.0
                ...
                pred = torch.argmax(resp_gmm, dim=-1)
                accuracy_gmm = torch.sum(pred == labels) / len(pred)
                pred = torch.argmax(resp_cmm, dim=-1)
                accuracy_cmm = torch.sum(pred == labels) / len(pred)
        ...
        avg_gmm_accuracy = np.mean(accuracy_gmm_list)
        avg_cmm_accuracy = np.mean(accuracy_cmm_list)
        ...
        train_stats.add(epoch, 'train_gmm_accuracy', avg_gmm_accuracy)
        train_stats.add(epoch, 'train_cmm_accuracy', avg_cmm_accuracy)
    ...
    def eval_model(self, model, pytorch_dataset, criterion, train_stats, split_name, epoch):
        ...
        with torch.no_grad():
            for batch_idx, tensor_dict in enumerate(dataloader):
                ...
                preds.extend(pred)
                pred = torch.argmax(resp_gmm, dim=-1).detach().cpu().numpy()
                preds_gmm.extend(pred)
                pred = torch.argmax(resp_cmm, dim=-1).detach().cpu().numpy()
                preds_cmm.extend(pred)
        ...
        softmax_accuracy = np.mean((np.asarray(preds) == gt_labels).astype(float))
        gmm_accuracy = np.mean((np.asarray(preds_gmm) == gt_labels).astype(float))
        cmm_accuracy = np.mean((np.asarray(preds_cmm) == gt_labels).astype(float))
        ...
        train_stats.add(epoch, '{}_cmm_accuracy'.format(split_name), cmm_accuracy)