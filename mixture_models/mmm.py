...
class GMM:
    ...
    def e_step(self):
        ...
        if self.converge:
            self.ll_new = torch.sum(torch.log(norm_vec)).item()
            if abs(self.ll_new - self.ll_prev) <= self.tolerance:
                return None
            self.ll_prev = self.ll_new
