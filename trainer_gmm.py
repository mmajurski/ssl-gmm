...
class FixMatchTrainer_gmm(trainer.SupervisedTrainer):
    ...
    def build_gmm(self, model, pytorch_dataset, skl=True):
        ...
        for i in range(len(unique_class_labels)):
            ...
            if not skl:
                ...
                gmm_skl = GMM_SKL(n_components=1, isCauchy=True)
                gmm_skl.fit(class_c_logits.numpy())
                gmm_list.append(gmm_skl)    
        ...
        # # merge the individual sklearn GMMs
            ...
            gmm = GMM_SKL(isCauchy=True, n_components=self.args.num_classes)
    ...
    def build_gmm_cmm(self, model, pytorch_dataset, skl=True):
        ...
        for i in range(len(unique_class_labels)):
            ...
            while np.any(np.isnan(cmm.get("sigma").detach().cpu().numpy())):
                cmm = GMM(n_features=class_c_logits.shape[1], n_clusters=1, tolerance=1e-4, max_iter=50, isCauchy=True)
                cmm.fit(class_c_logits)
            cmm_list.append(cmm)
        ...
        precisions = np.concatenate([gmm.precisions_ for gmm in gmm_list])
        precisions_chol = np.concatenate([gmm.precisions_cholesky_ for gmm in gmm_list])
        gmm = GMM_SKL(isCauchy=False, n_components=self.args.num_classes)
        gmm.weights_ = weights.numpy()
        gmm.means_ = means
        gmm.covariances_ = covariances
        gmm.precisions_ = precisions
        ...
        pi = torch.cat([torch.FloatTensor(cmm.weights_) for cmm in cmm_list])
        cmm_weights *= pi

        mus = torch.cat([cmm.get("mu") for cmm in cmm_list])
        sigmas = torch.cat([cmm.get("sigma") for cmm in cmm_list])
    ...
    #     self.gmm, self.cmm = self.build_gmm_cmm(model, pytorch_dataset, skl=self.args.skl)
    # else:
    ...
    ############# Older Version #####################
    def train_epoch_v1(self, model, pytorch_dataset, optimizer, criterion, epoch, train_stats, nb_reps=1, unlabeled_dataset=None):
    ...
    ############# Older Version #####################
    def eval_model_v1(self, model, pytorch_dataset, criterion, train_stats, split_name, epoch):
    ...
    def eval_model(self, model, pytorch_dataset, criterion, train_stats, split_name, epoch):
        ...
        dataloader = torch.utils.data.DataLoader(pytorch_dataset, batch_size=self.args.batch_size, shuffle=False,
                                                 num_workers=self.args.num_workers, worker_init_fn=utils.worker_init_fn)
        ...
        with torch.no_grad():
            for batch_idx, tensor_dict in enumerate(dataloader):
                ...
                #     if self.args.inference_method == 'gmm':
                #         gmm_resp = self.gmm.predict_proba(gmm_inputs.numpy())  # N*1, N*K
                #     elif self.args.inference_method == 'cauchy':
                #         _, _, gmm_resp = self.gmm.predict_cauchy_probability(gmm_inputs)
                #     if not isinstance(gmm_resp, np.ndarray):
                #         gmm_resp = gmm_resp.detach().cpu().numpy()
                ...
                if self.cmm is not None:
                    _, _, cmm_resp = self.gmm.predict_cauchy_probability(gmm_inputs)
                    cmm_resp = cmm_resp.detach().cpu().numpy()

                gmm_resp = ((torch.Tensor(self.alpha)) * gmm_resp) + ((torch.Tensor(1.0 - self.alpha)) * cmm_resp)
                ...
                if batch_idx % 100 == 0:
                    ...
                    gpu_mem_percent_used = [np.round(100 * x, 1) for x in gpu_mem_percent_used]
                    logging.info(
                        '  batch {}/{}  loss: {:8.8g}  cpu_mem: {:2.1f}%  gpu_mem: {}% of {}MiB'.format(batch_idx,
                                                                                                        batch_count,
                                                                                                        batch_loss.item(),
                                                                                                        cpu_mem_percent_used,
                                                                                                        gpu_mem_percent_used,
                                                                                                        memory_total_info))
