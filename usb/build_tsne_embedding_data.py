...
def get_config(config_fp):
    ...
    # inject the right config here
    parser.set_defaults(c=config_fp)
...
if __name__ == "__main__":
    ...
    subset = 'stl10' #'cifar10'
    ifp = '/home/mmajurski/github/ssl-gmm/models-tsne'
    fns = [fn for fn in os.listdir(ifp) if subset in fn]
    fns.sort()

    config_fp = '/home/mmajurski/github/ssl-gmm/usb/config/classic_cv/aagmm/{}/'.format(subset)

    for fn in fns:
        print(fn)
        cfp = os.path.join(config_fp, fn + '.yaml')
        if not os.path.exists(cfp):
            raise RuntimeError('Config file does not exist: {}'.format(cfp))
        ...
        args.resume = True
        args.overwrite = True
        args.load_path = os.path.join(ifp, fn, 'model_best.pth')
        if not os.path.exists(args.load_path):
            raise RuntimeError('Trained model file does not exist: {}'.format(args.load_path))
