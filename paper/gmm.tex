...
% TODO From Peter:
% From the paper about FixMatch:
% "Consistency regularization utilizes unlabeled data by relying on the assumption that the model
% should output similar predictions when fed perturbed versions of the same image."
% "Pseudo-labeling leverages the idea of using the model itself to obtain artificial labels for unlabeled
% data [ 32 , 47]. Specifically, this refers to the use of “hard” labels (i.e., the arg max of the model’s
% output) and only retaining artificial labels whose largest class probability fall above a predefined
% threshold [25]."
% Q1: Is your contribution in avoiding the thresholding in FixMatch when pseudo-labeling?
% Q2: is your algorithm assuming a minimum distance separation of classes based on the consistency regularization?
% Q3: Is your approach assuming that classes are represented by convex shapes in high-dimensional manifolds and therefore clusters (and their centers) are good descriptors of classes?

% In the nutshell, can you add a paragraph with your assumptions about the data representation? 
...
% TODO From Peter
% how do you choose weights? Do they have to sum to one? what if I choose N moments as opposed to 4 moments? 
